...
if false % skip simulation and instead load previously simulated data
    tic;
    for n=1:length(nHill)
        k(7)=nHill(n); k(8)=nHill(n);
        for r=1:length(RR)
            [t_semi(r,n,:),c_semi(r,n,:,:)]=...
                semiContinuousODE(@(t,y)ode_simple_mRNA(t,y,k,0),t_sec,y0,RR(r),generateInput(inPeriod/t_interval,inAmplitude,t_sec,y0),semiOut);
        end
...
%save('211108_sim_data.mat','c_semi','delta','RR','nHill','RR','lambda','t_hour','t_semi','rotation','-v7.3');